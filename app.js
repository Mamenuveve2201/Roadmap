...
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV2cWpzd2RmY2l4d2t0Z3ZuemJxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIzNDExMzEsImV4cCI6MjA4NzkxNzEzMX0.SMB9fnxmLHFNTNkJk47UFpMqlccv_wvJOYAicvGxqoI';
const _SID = '46ff1227-8bce-4138-9993-3b9f6f2508ef';
const _SH  = { 'apikey': SUPABASE_ANON, 'Authorization': 'Bearer ' + SUPABASE_ANON, 'Content-Type': 'application/json' };
...
const TASKS = {
  python: [
    { phase: "Week 1 — Setup + Data Generation (Mar 19–23)", dates: "Mar 19–23", items: [
  ...
  security: [
    { phase: "Week 1 — Domain 1: General Security Concepts (Mar 23–29)", dates: "Mar 23–29 · 12% of exam", items: [
      ...
      "SAT 03/28 late night: 🃏 Quizlet port numbers — run deck until 90%+",
    ...
    { phase: "Week 2 — Domain 2: Threats, Vulnerabilities & Mitigations (Mar 30 – Apr 5)", dates: "Mar 30 – Apr 5 · 22% of exam", items: [
      "MON 03/30 8pm–11:30pm: 📺 Prof. Messer 2.1–2.2 — Threat actors + Threat vectors",
    ...
    { phase: "Week 3 — Domain 3: Security Architecture (Apr 6–12)", dates: "Apr 6–12 · 18% of exam", items: [
      ...
      "SAT 04/11 late night: 🃏 Quizlet ports (aim 95%+) + 📺 CyberKraft cloud/architecture PBQs + ExamCompass mixed test",
      "SUN 04/12 late night: Review ALL wrong answers from ExamCompass — understand WHY each was wrong",
    ]},
    { phase: "Week 4 — Domains 4 + 5: Operations & Governance (Apr 13–20)", dates: "Apr 13–20 · Domain 4=28%, Domain 5=20%", items: [
      "MON 04/13 8pm–11:30pm: 📺 Prof. Messer 4.1–4.2 — IAM (MFA, SSO, PAM, LDAP/AD) + Vulnerability scanning (Nessus, OpenVAS)",
      "TUE 04/14 8pm–11:30pm: 📺 Prof. Messer 4.3–4.4 — SIEM, log analysis, Incident Response lifecycle",
      "WED 04/15 8pm–11:30pm: 📺 Prof. Messer 4.5–4.6 — Endpoint security (EDR, MDM, MAM, BYOD) + Automation (SOAR) + Digital forensics",
      "THU 04/16 8pm–11:30pm: 📺 Prof. Messer 5.1–5.6 — Risk management, compliance frameworks (NIST, ISO 27001, PCI-DSS, HIPAA, GDPR)",
      "SAT 04/18 late night: 🃏 Quizlet ports (must be 100%) + 📺 CyberKraft Domain 4/5 PBQs + ExamCompass full mixed test",
      "SUN 04/19 late night: ⏱️ FULL timed practice exam — 90 questions, 90 min (ExamCompass) · review every wrong answer",
    ]},
  ...
  braiding: [
    { phase: "Knotless Braids (Mar 23 – Apr 2)", dates: "Mar 23 – Apr 2 · 30 min on Mon or Wed", items: [
    ...
    { phase: "Box Braids (Apr 6–30)", dates: "Apr 6–30 · 30 min on Mon or Wed", items: [
    ...
    { phase: "🔒 Cornrows (May 2026) — Unlocks Phase 2", dates: "May 2026+", items: [
    ...
    { phase: "🔒 Stitch Braids (Jun 2026) — Unlocks Phase 2", dates: "Jun 2026+", items: [
    ...
    { phase: "🔒 Rope Twists (Jul 2026) — Unlocks Phase 2", dates: "Jul 2026+", items: [
    ...
    { phase: "🔒 More Styles (Aug 2026+) — Unlocks Phase 2", dates: "Aug 2026+", items: [
  ...
  jarvis: [
    { phase: "🔒 v0.1 — Core Voice AI (May 2026)", dates: "May 2026+", items: [
    ...
    { phase: "v0.2 — Computer Control (Jun 2026)", dates: "Jun 2026", items: [
    ...
    { phase: "v0.3 — API Integrations (Jul 2026)", dates: "Jul 2026", items: [
    ...
    { phase: "v0.4 — Memory System (Aug 2026)", dates: "Aug 2026", items: [
    ...
    { phase: "v0.5–1.0 — Polish + Local LLM (Oct–Dec 2026)", dates: "Oct–Dec 2026", items: [
  ...
  hacking: [
    { phase: "🔒 Foundations (May–Jun 2026)", dates: "May–Jun 2026+", items: [
  ...
  languages: [
    { phase: "🔒 French — Foundations (May–Jul 2026)", dates: "May 2026+ · 30 min/day", items: [
    ...
    { phase: "🇪🇸 Spanish — Foundations (Aug–Sep 2026)", dates: "Aug–Sep 2026", items: [
    ...
    { phase: "🇪🇸 Spanish — Conversational (Oct–Nov 2026)", dates: "Oct–Nov 2026", items: [
    ...
    { phase: "🤟 ASL — Foundations (Dec 2026 – Jan 2027)", dates: "Dec 2026–Jan 2027", items: [
    ...
    { phase: "🤟 ASL — Conversational (Feb–Mar 2027)", dates: "Feb–Mar 2027", items: [
...
let customTasks = {};   // { trackKey: [{phase, dates, items:[]}] }
let deadlines   = {     // editable deadlines stored in cloud
  securityExam:   '2026-04-21',
  pythonProject:  '2026-04-23',
  phase2Start:    '2026-05-01',
};
...
async function syncFromCloud() {
  ...
  try {
    const r = await fetch(SUPABASE_URL + '/rest/v1/user_data?id=eq.' + _SID + '&select=roadmap_data', { headers: _SH });
    if (!r.ok) throw new Error(r.status);
    const rows = await r.json();
    const d = rows && rows[0] && rows[0].roadmap_data;
    if (d) {
      if (d.tasksDone)   tasksDone   = d.tasksDone;
      if (d.sessions)    sessions    = d.sessions;
      if (d.notes)       notes       = d.notes;
      if (d.streak)      streak      = d.streak;
      if (d.customTasks) customTasks = d.customTasks;
      if (d.deadlines)   deadlines   = { ...deadlines, ...d.deadlines };
      saveLocal('tasksDone',   tasksDone);
      saveLocal('sessions',    sessions);
      saveLocal('notes',       notes);
      saveLocal('streak',      streak);
      saveLocal('customTasks', customTasks);
      saveLocal('deadlines',   deadlines);
    } else if (sessions.length > 0 || Object.keys(tasksDone).length > 0) {
      await pushAllToCloud(); return;
    }
...
async function pushAllToCloud() {
  try {
    const payload = { roadmap_data: { tasksDone, sessions, notes, streak, customTasks, deadlines, updatedAt: new Date().toISOString() } };
    const r = await fetch(SUPABASE_URL + '/rest/v1/user_data?id=eq.' + _SID, {
      method: 'PATCH', headers: _SH, body: JSON.stringify(payload)
    });
    showSyncStatus(r.ok ? 'synced' : 'offline');
...
function showSyncStatus(status) {
  ...
  else                           { el.textContent = '⚠️'; el.title = 'Offline — check connection'; }
}
...
// ============================================================
// PROGRESS — includes customTasks
// ============================================================
function getAllPhasesForTrack(track) {
  const built = TASKS[track] || [];
  const custom = (customTasks[track] || []).map(g => ({
    phase: g.phase,
    dates: g.dates,
    items: g.items,
    _custom: true,
...
function calcProgress(track) {
  const phases = getAllPhasesForTrack(track);
  let total = 0, done = 0;
  phases.forEach((phase, pi) => {
    phase.items.forEach((_, ti) => {
      total++;
      if (tasksDone[`${track}-${pi}-${ti}`]) done++;
    });
  });
...
function renderDashboard() {
  ...
  const phase2Date = new Date(deadlines.phase2Start || '2026-05-01');
  const phase2Banner = document.getElementById('phase-banner');
  if (new Date() < phase2Date) {
    const daysLeft = Math.ceil((phase2Date - new Date()) / 86400000);
    phase2Banner.innerHTML = `📅 <strong>Phase 1 active</strong> — Python, Security+ & Braiding. Phase 2 unlocks in <strong>${daysLeft} days</strong>.`;
  ...
  const secDate  = new Date(deadlines.securityExam  || '2026-04-21');
  const projDate = new Date(deadlines.pythonProject  || '2026-04-23');
  const secDays  = Math.ceil((secDate  - new Date()) / 86400000);
  const projDays = Math.ceil((projDate - new Date()) / 86400000);
  const deadlineEl = document.getElementById('deadline-banner');
  const fmt = d => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
  if (secDays > 0 && secDays <= 60 && projDays > 0) {
    deadlineEl.innerHTML = `⏰ Security+ in <strong>${secDays}d</strong> (${fmt(secDate)}) · Python project in <strong>${projDays}d</strong> (${fmt(projDate)})`;
    deadlineEl.style.display = 'block';
  } else if (projDays > 0 && projDays <= 60) {
    deadlineEl.innerHTML = `⏰ Pattern Recognition project due in <strong>${projDays} days</strong> (${fmt(projDate)})`;
    deadlineEl.style.display = 'block';
  } else {
  ...
  Object.keys(TRACK_META).forEach(t => {
    ...
    const locked = TRACK_META[t].phase === 2 && new Date() < phase2Date;
    if (bar)   bar.style.width   = pct + '%';
    if (pctEl) pctEl.textContent = locked ? '🔒 Phase 2' : pct + '%';
    if (card)  card.style.opacity = locked ? '0.45' : '1';
  });
...
// ============================================================
// RENDER TASKS — built-in + custom merged
// ============================================================
function renderTasks(track) {
  ...
  if (meta.phase === 2 && new Date() < phase2Date) {
    container.innerHTML = `<div class="empty-state" style="padding:40px 20px;text-align:center">
      🔒 <strong>${meta.icon} ${meta.label}</strong> unlocks Phase 2.<br><br>
      Focus on Python, Security+ and Braiding for now. You've got this! 💪
    </div>
    <div style="text-align:center;margin-top:16px">
      <button onclick="openManageModal('${track}')" style="background:var(--accent);color:#fff;border:none;border-radius:8px;padding:8px 18px;font-size:13px;cursor:pointer;font-family:inherit">+ Add Goal Anyway</button>
    </div>`;
    return;
  }

  const allPhases = getAllPhasesForTrack(track);
  let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
    <div style="color:var(--text2);font-size:13px">Progress: <strong style="color:${meta.color}">${calcProgress(track)}% complete</strong></div>
    <button onclick="openManageModal('${track}')" style="background:var(--accent);color:#fff;border:none;border-radius:8px;padding:7px 14px;font-size:12px;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:5px">＋ Add Goal / Task</button>
  </div>`;
  ...
  allPhases.forEach((phase, pi) => {
    ...
    const isCustom  = !!phase._custom;

    html += `<div class="phase-group ${isLocked ? 'phase-locked' : ''}">
      <div class="phase-header">
        <div style="flex:1">
          <div class="phase-title">${phase.phase}${isCustom ? ' <span style="font-size:10px;background:var(--accent);color:#fff;padding:1px 5px;border-radius:4px;vertical-align:middle">custom</span>' : ''}</div>
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          ${isCustom ? `<button onclick="deleteCustomGoal('${track}','${phase._id}')" style="background:none;border:none;cursor:pointer;color:var(--text2);font-size:14px;padding:2px 4px" title="Delete goal">🗑</button>` : ''}
        </div>
      </div>`;
...
// ============================================================
// MANAGE MODAL — Add goals, tasks, edit deadlines
// ============================================================
function openManageModal(track) {
  currentTrack = track;
  const modal = document.getElementById('manage-modal');
  if (!modal) { buildManageModal(); }
  populateManageModal(track);
...
function buildManageModal() {
  ...
  modal.innerHTML = `
    <div class="modal-inner">
      <div class="modal-header-row">
        <div id="manage-modal-title" style="font-size:16px;font-weight:700;color:var(--text)">Manage Roadmap</div>
        <button onclick="document.getElementById('manage-modal').classList.remove('open')" style="background:none;border:none;cursor:pointer;color:var(--text2);font-size:20px">✕</button>
      </div>

      <div style="display:flex;gap:6px;margin:14px 0 18px">
        <button class="manage-tab-btn active" data-tab="goal" onclick="switchManageTab('goal')">+ New Goal</button>
        <button class="manage-tab-btn" data-tab="task" onclick="switchManageTab('task')">+ New Task</button>
        <button class="manage-tab-btn" data-tab="dates" onclick="switchManageTab('dates')">📅 Dates</button>
      </div>

      <!-- ADD GOAL TAB -->
      <div id="manage-tab-goal">
        <div class="form-row">
          <label class="form-label">Track</label>
          <select id="mg-track" class="form-input" onchange="populateGoalPhases()">
            ${Object.entries(TRACK_META).map(([k,v]) => `<option value="${k}">${v.icon} ${v.label}</option>`).join('')}
          </select>
        </div>
        <div class="form-row">
          <label class="form-label">Goal Name / Phase Title</label>
          <input id="mg-phase" class="form-input" placeholder="e.g. Advanced Python (May 2026)" />
        </div>
        <div class="form-row">
          <label class="form-label">Dates / Timeframe</label>
          <input id="mg-dates" class="form-input" placeholder="e.g. May–Jun 2026" />
        </div>
        <div class="form-row">
          <label class="form-label">Tasks (one per line)</label>
          <textarea id="mg-items" class="form-input" rows="5" placeholder="Task 1&#10;Task 2&#10;Task 3"></textarea>
        </div>
        <button onclick="saveCustomGoal()" class="btn-primary" style="width:100%;margin-top:4px">Save Goal</button>
      </div>

      <!-- ADD TASK TAB -->
      <div id="manage-tab-task" style="display:none">
        <div class="form-row">
          <label class="form-label">Track</label>
          <select id="mt-track" class="form-input" onchange="populateTaskPhases()">
            ${Object.entries(TRACK_META).map(([k,v]) => `<option value="${k}">${v.icon} ${v.label}</option>`).join('')}
          </select>
        </div>
        <div class="form-row">
          <label class="form-label">Goal / Phase</label>
          <select id="mt-phase" class="form-input"></select>
        </div>
        <div class="form-row">
          <label class="form-label">Task Text</label>
          <input id="mt-item" class="form-input" placeholder="e.g. Complete chapter 4 exercises" />
        </div>
        <button onclick="saveCustomTask()" class="btn-primary" style="width:100%;margin-top:4px">Add Task</button>
      </div>

      <!-- EDIT DATES TAB -->
      <div id="manage-tab-dates" style="display:none">
        <p style="font-size:12px;color:var(--text2);margin-bottom:14px">Edit key dates. Changes sync to all your devices.</p>
        <div class="form-row">
          <label class="form-label">Security+ Exam Date</label>
          <input id="dd-security" type="date" class="form-input" />
        </div>
        <div class="form-row">
          <label class="form-label">Python Project Due Date</label>
          <input id="dd-python" type="date" class="form-input" />
        </div>
        <div class="form-row">
          <label class="form-label">Phase 2 Start Date</label>
          <input id="dd-phase2" type="date" class="form-input" />
        </div>
        <button onclick="saveDeadlines()" class="btn-primary" style="width:100%;margin-top:4px">Save Dates</button>
      </div>
    </div>`;
  ...
  if (!document.getElementById('manage-modal-styles')) {
    const s = document.createElement('style');
    s.id = 'manage-modal-styles';
    s.textContent = `
      #manage-modal { display:none; position:fixed; inset:0; background:rgba(0,0,0,.6); z-index:500; align-items:flex-end; justify-content:center; }
      #manage-modal.open { display:flex; }
      #manage-modal .modal-inner { background:var(--bg2); border-radius:18px 18px 0 0; padding:22px 20px 36px; width:100%; max-width:520px; max-height:88vh; overflow-y:auto; }
      .manage-tab-btn { flex:1; padding:8px 10px; border:1px solid var(--border); background:var(--bg3); color:var(--text2); border-radius:8px; cursor:pointer; font-size:12px; font-family:inherit; transition:all .15s; }
      .manage-tab-btn.active { background:var(--accent); color:#fff; border-color:var(--accent); }
      .modal-header-row { display:flex; justify-content:space-between; align-items:center; }
      .btn-primary { background:var(--accent); color:#fff; border:none; border-radius:10px; padding:13px; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; }
    `;
...
function switchManageTab(tab) {
  ['goal','task','dates'].forEach(t => {
    document.getElementById(`manage-tab-${t}`).style.display = t === tab ? 'block' : 'none';
    document.querySelector(`.manage-tab-btn[data-tab="${t}"]`)?.classList.toggle('active', t === tab);
  });
  if (tab === 'task') populateTaskPhases();
  if (tab === 'dates') {
    document.getElementById('dd-security').value = deadlines.securityExam  || '';
    document.getElementById('dd-python').value   = deadlines.pythonProject || '';
    document.getElementById('dd-phase2').value   = deadlines.phase2Start   || '';
  }
}

function populateManageModal(track) {
  const mgTrack = document.getElementById('mg-track');
  const mtTrack = document.getElementById('mt-track');
  if (mgTrack) mgTrack.value = track;
  if (mtTrack) { mtTrack.value = track; populateTaskPhases(); }
  switchManageTab('goal');
}

function populateGoalPhases() { /* just sets track for goal — no phase needed */ }

function populateTaskPhases() {
  const track  = document.getElementById('mt-track')?.value;
  const sel    = document.getElementById('mt-phase');
  if (!sel || !track) return;
  const phases = getAllPhasesForTrack(track);
  sel.innerHTML = phases.map((p, i) => `<option value="${i}">${p.phase}</option>`).join('');
}

async function saveCustomGoal() {
  const track = document.getElementById('mg-track').value;
  const phase = document.getElementById('mg-phase').value.trim();
  const dates = document.getElementById('mg-dates').value.trim();
  const raw   = document.getElementById('mg-items').value.trim();
  if (!phase) return showToast('⚠️ Enter a goal name');
  if (!raw)   return showToast('⚠️ Enter at least one task');

  const items = raw.split('\n').map(s => s.trim()).filter(Boolean);
  if (!customTasks[track]) customTasks[track] = [];
  customTasks[track].push({ id: 'cg-' + Date.now(), phase, dates: dates || 'Custom', items });

  saveLocal('customTasks', customTasks);
  await pushAllToCloud();
  document.getElementById('mg-phase').value = '';
  document.getElementById('mg-dates').value = '';
  document.getElementById('mg-items').value = '';
...
async function saveCustomTask() {
  const track   = document.getElementById('mt-track').value;
  const phaseIdx = parseInt(document.getElementById('mt-phase').value);
  const itemText = document.getElementById('mt-item').value.trim();
  if (!itemText) return showToast('⚠️ Enter a task');

  const allPhases = getAllPhasesForTrack(track);
  const phase     = allPhases[phaseIdx];
  if (!phase) return showToast('⚠️ Invalid phase');

  if (phase._custom) {
    // Add to custom goal
    const cg = (customTasks[track] || []).find(g => g.id === phase._id);
    if (cg) cg.items.push(itemText);
  } else {
    // Add as a new custom goal that shadows/extends this track at a custom index
    // We'll attach it to a "__extra" custom goal
    if (!customTasks[track]) customTasks[track] = [];
    let extra = customTasks[track].find(g => g.id === `__extra_${track}`);
    if (!extra) {
      extra = { id: `__extra_${track}`, phase: `Extra Tasks — ${TRACK_META[track].label}`, dates: 'Added in-app', items: [] };
      customTasks[track].push(extra);
    }
    extra.items.push(itemText);
  }

  saveLocal('customTasks', customTasks);
  await pushAllToCloud();
  document.getElementById('mt-item').value = '';
  document.getElementById('manage-modal').classList.remove('open');
  renderTasks(track);
  showToast('✅ Task added!');
}

async function deleteCustomGoal(track, goalId) {
  if (!confirm('Delete this custom goal and all its tasks?')) return;
  if (customTasks[track]) {
    customTasks[track] = customTasks[track].filter(g => g.id !== goalId);
  }
  // Clean up any tasksDone keys that belong to deleted phases
  // (They'll just become orphaned which is fine — no visible harm)
  saveLocal('customTasks', customTasks);
  await pushAllToCloud();
  renderTasks(track);
...
async function saveDeadlines() {
  deadlines.securityExam  = document.getElementById('dd-security').value || deadlines.securityExam;
  deadlines.pythonProject = document.getElementById('dd-python').value   || deadlines.pythonProject;
  deadlines.phase2Start   = document.getElementById('dd-phase2').value   || deadlines.phase2Start;
  saveLocal('deadlines', deadlines);
  await pushAllToCloud();
  document.getElementById('manage-modal').classList.remove('open');
  renderCurrentPage();
...
async function saveLog() {
  ...
  const newSession = { date, track, hours, notes: notesVal, mood: selectedMood || '', ts: Date.now(), id: 'ses-' + Date.now() };
...
function openNoteModal(noteId = null) {
  ...
  if (noteId) {
    ...
    if (n) {
      ...
      document.getElementById('note-track-input').value = n.track || 'general';
    }
  } else {
...
async function saveNote() {
  ...
  if (editingNoteId) {
    ...
    if (idx !== -1) notes[idx] = { ...notes[idx], title, body, track };
    showToast('📝 Note updated');
  } else {
    notes.push({ title, body, track, ts: Date.now(), id: 'note-' + Date.now() });
...
// ============================================================
// BOOT — no auth, straight to app
// ============================================================
document.addEventListener('DOMContentLoaded', async () => {

  // Load local data first so UI isn't empty while syncing
  tasksDone   = loadLocal('tasksDone',   {});
  sessions    = loadLocal('sessions',    []);
  notes       = loadLocal('notes',       []);
  streak      = loadLocal('streak',      { count: 0, best: 0, lastDate: null });
  customTasks = loadLocal('customTasks', {});
  deadlines   = loadLocal('deadlines',   deadlines);

  // Show app immediately — no auth wall
  ...
  document.getElementById('sync-btn')?.addEventListener('click', async () => {
    await syncFromCloud(); showToast('☁️ Synced!');
  });
  ...
  // Settings btn → reset local / open manage
  document.getElementById('settings-btn')?.addEventListener('click', () => {
    openManageModal(currentTrack || 'python');
  });
  ...
  document.querySelectorAll('.progress-card[data-track]').forEach(card => {
    card.addEventListener('click', () => {
      ...
      navigateTo('tasks');
      setTimeout(() => {
        const tab = document.querySelector(`.track-tab[data-track="${track}"]`);
        if (tab) {
          document.querySelectorAll('.track-tab').forEach(t => t.classList.remove('active'));
          tab.classList.add('active');
          currentTrack = track;
