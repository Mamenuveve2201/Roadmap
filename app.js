...
async function syncFromCloud() {
  ...
  try {
    ...
    if (d) {
      ...
      saveLocal('tasksDone',tasksDone); saveLocal('sessions',sessions);
      saveLocal('notes',notes); saveLocal('streak',streak);
    } else if (sessions.length > 0 || Object.keys(tasksDone).length > 0) {
      await pushAllToCloud(); return;
    }
    showSyncStatus('synced'); renderCurrentPage();
  } catch(e) { console.error(e); showSyncStatus('offline'); }
}
async function pushAllToCloud() {
  try {
    const r = await fetch(SUPABASE_URL+'/rest/v1/user_data?id=eq.'+_SID, {
      method:'PATCH', headers:_SH,
      body:JSON.stringify({roadmap_data:{tasksDone,sessions,notes,streak,updatedAt:new Date().toISOString()}})
    });
    showSyncStatus(r.ok ? 'synced' : 'offline');
  } catch(e) { showSyncStatus('offline'); }
}

async function pushTaskDone(key, isDone) { await pushAllToCloud(); }
async function pushSession(s) { await pushAllToCloud(); return s.id; }
async function deleteSessionCloud(id) { await pushAllToCloud(); }
async function pushNote(note) { await pushAllToCloud(); return note.id; }
async function updateNoteCloud(note) { await pushAllToCloud(); }
async function deleteNoteCloud(id) { await pushAllToCloud(); }
async function pushStreak() { await pushAllToCloud(); }
...
document.addEventListener('DOMContentLoaded', async () => {
  // No auth needed — hardcoded UUID syncs to My-First-Budget-App Supabase
  currentUser = { id: '46ff1227-8bce-4138-9993-3b9f6f2508ef', email: 'melissa@local' };
  renderAppShell();
  await syncFromCloud();
